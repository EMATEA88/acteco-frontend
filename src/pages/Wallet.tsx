...
export default function Wallet() {
  ...
  const handleSave = async () => {
    ...
    // 🔥 VALIDAÇÃO BSC (CORRETA)
    if (!cleanAddress.startsWith("0x") || cleanAddress.length !== 42) {
      return toast.error("Endereço BSC (BEP20) inválido!")
    }
    ...
    try {
      await UserService.updateProfile({
        withdrawWalletAddress: cleanAddress
      });
      ...
      <div className="space-y-6">
        ...
        <div className="bg-[#161A1F] p-6 rounded-[1.75rem] border border-white/5 space-y-4 shadow-2xl">
          ...
          <div className="flex items-center gap-3 text-cyan-400">
            ...
            <span className="text-xs font-bold uppercase tracking-widest">
              Rede Segura: BSC (BEP20)
            </span>
          </div>

          <div className="space-y-2">
            <p className="text-sm text-gray-400">
              Insira seu endereço USDT (BEP20) para receber pagamentos.
            </p>
        ...
        <div className="p-4 bg-amber-500/5 border border-amber-500/10 rounded-2xl">
          <p className="text-[10px] text-amber-500 leading-relaxed uppercase font-black text-center tracking-wider">
            Atenção: A EMATEA opera exclusivamente via USDT na rede BSC (BEP20).
